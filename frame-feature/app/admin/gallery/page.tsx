...
export default function NewGalleryItemPage() {
  ...
  const [iframeEmbed, setIframeEmbed] = useState("");
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    if (mediaType === "iframe" && !iframeEmbed.trim()) {
      setErrorMsg("Please provide the iFrame HTML embed code.");
      return;
    }
      ...
      try {
        const res = await fetch("/api/gallery", {
          ...
          body: JSON.stringify({
            ...
            mediaType,
            iframeEmbed: mediaType === "iframe" ? iframeEmbed.trim() : "",
          }),
        });

        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.error || "Failed to upload visual.");
        }

        setSuccessMsg("Visual uploaded successfully! Redirecting to dashboard...");
        setTimeout(() => {
          router.push("/admin");
          router.refresh();
        }, 1000);
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-950 border border-white/10 text-sm text-white focus:outline-none focus:border-[#FF5E14]"
                      >
                        <option value="image">Standard Image / Photo</option>
                        <option value="iframe">Interactive iFrame Embed</option>
                      </select>
                    </div>
                  ...
                  {mediaType === "iframe" && (
                    <div>
                      ...
                      <textarea
                        required
                        rows={3}
                        placeholder='e.g. <iframe src="https://..." width="100%" height="300"></iframe>'
                        value={iframeEmbed}
                        onChange={(e) => setIframeEmbed(e.target.value)}
                      ...
                      <p className="text-[10px] text-zinc-500 mt-1">
                        Paste the full HTML embed tag.
                      </p>
                    </div>
                  )}

                  <ImageUploadInput
                    label={mediaType === "iframe" ? "Thumbnail Cover Image" : "Visual Frame Image"}
                    folder="gallery"
                    value={image}
                    onChange={setImage}
                  />
